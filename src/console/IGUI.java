...
public abstract class IGUI {
    ...
    /*Initializers*/
    public void loadWithFile(String path) {
        FS = new FileSystemGUI(new FS_File(path));
    }

    public void loadNewFile() {
        // solicita tamaño
        this.print("disk size: ");
        String size = this.getArgumentFromConsole();
        String[] sizeArguments = size.split("\\s+");
        if (sizeArguments.length != 1 && !sizeArguments[0].equals("")) {
            this.print("incorrect size");
        } else {
            int diskSize = 0;
            try {
                 diskSize = Integer.parseInt(sizeArguments[0]);
            } catch (Exception ex) {
                this.print("disk size must be a number");
            }
            this.print("password: ");
            String pass = this.getArgumentFromConsole();
            String[] passArguments = pass.split("\\s+");
            if (passArguments.length != 1 && !passArguments[0].equals("")) {
                this.print("invalid password");
            } else {
                try {
                    FS = new FileSystemGUI(diskSize, passArguments[0], new FS_File(diskSize));
                    mainMenu();
                } catch (AWTException ex) {
                    Logger.getLogger(IGUI.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
